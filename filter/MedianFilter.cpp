...
namespace imaging {
	...
	MedianFilter::MedianFilter(Image * source) :RegionFilter(source) {}

	MedianFilter::~MedianFilter() {}

	void MedianFilter::apply_filter() {
		...
		for (unsigned i = 0; i < height; i++) {
			for (unsigned j = 0; j < width; j++) {
				...
				Color median = Color(r_channel[(V - 1) / 2], g_channel[(V - 1) / 2], b_channel[(V - 1) / 2]);	//find the median of each channel and make a Color with the values
				...
				for (unsigned n = 0; n < 3; n++) {
					for (unsigned m = 0; m < 3; m++) {
						if (pixel_arr[n][m] == Color(component_t(0), component_t(0), component_t(0))) continue;	//if default value continue loop
						img_buffer[(i - 1 + n)*width + (j - 1 + m)] = median;									//change the pixel in the Image with the median Color
					}
				}
			}
		}
